...
from my_libs.OB1L1B import output_time, date_range_str, is_before_date, log_activity
...
class Player:
    ...
    def get_result(self, count_days):
        avg_online_time, total_online_time = output_time(
            self.online_time/count_days), output_time(self.online_time)
        avg_vanish_time, total_vanish_time = output_time(
            self.vanish_time/count_days), output_time(self.vanish_time)
        avg_online_without_vanish_time, total_online_without_vanish_time = output_time(
            (self.online_time/count_days) - self.vanish_time/count_days), output_time(self.online_time - self.vanish_time)
        return {
                ...
                for line in file:
                    for player in players:
                        if player in line and log_activity(line.split(), activity[player], date):
                            break
        for player in players:
            activity[player].local_msg = 0
            activity[player].global_msg = 0
            activity[player].private_msg = 0
            activity[player].warns = 0
            activity[player].kicks = 0
            activity[player].mutes = 0
            activity[player].bans = 0
            activity[player].online_time = 0
            activity[player].vanish_time = 0
            activity[player].online_status = False
            activity[player].vanish_status = False
    for date in days:
        if f'{date}.txt' not in local_logs:
            not_found_logs.append(date)
            continue
