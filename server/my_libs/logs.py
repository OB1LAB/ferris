...
def get_log_data(line):
    ...
    if len(log) > 5 and data_in_brackets and log[5] not in ['[L]', '[G]'] and contains_log(data_in_brackets,
    ...
    elif len(log) > 2 and data_in_brackets and (len(log) < 6 or log[5] not in ['[L]', '[G]']) and contains_log(data_in_brackets,
                                                            ['main/DEBUG', 'main/INFO', 'main/TRACE', 'main/WARN,'
                                                             'main/ERROR']):
        data_log['type'] = 'Main information'
    elif len(log) > 2 and data_in_brackets and (len(log) < 6 or log[5] not in ['[L]', '[G]']) and contains_log(data_in_brackets, ['DEBUG', 'INFO', 'TRACE', 'WARN']):
...
def get_value_macros():
    data = {}
    keys = get_macro_keys()
    macros = read_file(f'{Config.minecraft_path}/liteconfig/common/macros/.macros.txt')
